...
@Component({
  selector: 'app-cxc-view',
  templateUrl: './cxc-view.component.html',
  styleUrls: ['./cxc-view.component.css']
})
export class CxcViewComponent implements OnInit {
  ...
  getUpcomingAccounts(filter : any){
    this.reportService.getCXC(this.slug,filter).subscribe(
      res =>{
        if(res != null){
          let temp : any = [];
          temp = res;
          if(temp.accounts != undefined){
            ...
            Swal.fire({
              title: 'Cuentas por cobrar próximas a vencer en 30 días',
              html: `${show}`,
            ...
            this.buttonFilter = false;
            this.showmodalError("Esta empresa no tiene con cuentas por cobrar.");
          }
        }
      },err=>{
        console.log(err);
  ...
  getAccounts(fil : any){
    ...
    let timeoutId;
    this.reportService.getCXC(this.slug,fil).subscribe(
      res =>{
        if(res != null){
          let temp : any = [];
          temp = res;
