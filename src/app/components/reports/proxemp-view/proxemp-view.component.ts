...
@Component({
  ...
  templateUrl: './proxemp-view.component.html',
  styleUrls: ['./proxemp-view.component.css']
})
export class ProxempViewComponent implements OnInit {
  coll : boolean = false;
  ...
  constructor(private companyService:CompanyServiceService,private storageService : StorageServiceService,private providerService : ProviderServiceService,private reportService : ReportServiceService) { }
  ...
  onClickDownloadPdf(){
    if(this.slug != null || this.slug != undefined){
      ...
      this.reportService.getPROXCOMPPDF(this.slug).subscribe(
        res =>{
          if(res != null){
            let temp : any;
            temp = res;
            let base64String =temp.report.toString();
            this.downloadPdf(base64String,"Proveedores por empresa");
          }
        },err =>{
          console.log(err);
          this.showmodalError(err.error.message);        
